...
def run_test_1():
    ...
    csv_files = glob.glob(os.path.join(extracted_dir, "**", "[1-4]T20??.csv"), recursive=True)
    csv_files = sorted(csv_files)

    if not csv_files:
        raise RuntimeError("Nenhum CSV trimestral encontrado em data/extracted. Verifique se a extração funcionou.")
